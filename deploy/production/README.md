...
- systemd service: `/etc/systemd/system/celery_anglangl.service`
...
- Celery worker must run under systemd and subscribe to `default,youtube_download,clip_extract,clip_upload_process`.
- Secrets must live only in `/etc/anglangl/anglangl.env`.
- Gunicorn process must run as user `cskang` and group `www-data`.
- Celery worker process must run as user `cskang` and group `www-data`.
